...
# --- SIDEBAR: GOOGLE CALENDAR & CONTROLS ---
with st.sidebar:
...
def fetch_imd_7day_forecast():
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if target_station:
                forecast_days = target_station.get("forecast") or target_station.get("forecastData") or []
                parsed_rows = []
                for day in forecast_days[:7]:
                    parsed_rows.append({
                        "Date": day.get("date") or day.get("Date") or "N/A",
                        "Max Temp (°C)": day.get("max_temp") or day.get("temp_max") or "N/A",
                        "Min Temp (°C)": day.get("min_temp") or day.get("temp_min") or "N/A",
                        "Weather": day.get("weather_description") or day.get("forecast") or "Clear"
                    })
                if parsed_rows:
